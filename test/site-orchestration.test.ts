...
function createShellFixture(existing = true): { root: string; log: string; environment: NodeJS.ProcessEnv } {
  ...
  fake("npx", [
    ...
    'case "$*" in',
    '  *render-runtime-env.ts\ write*) mkdir -p "$(dirname "$5")"; printf "POSTGRES_MODE=neon\\nREDIS_MODE=upstash\\n" > "$5" ;;',
    '  *render-site-route.ts\ write*) mkdir -p "$(dirname "$6")"; printf "route\\n" > "$6" ;;',
    '  *write-deploy-state.ts\ write*) mkdir -p "$(dirname "$5")"; printf "%s\\n" "$6" > "$5" ;;',
    '  *write-bootstrap-marker.ts\ write*) mkdir -p "$(dirname "$5")"; printf "marker\\n" > "$5" ;;',
    '  *write-host-state.ts\ write*) mkdir -p "$(dirname "$5")"; printf "{\\\"sites\\\":[\\\"code2\\\",\\\"code3\\\"]}\\n" > "$5" ;;',
    'esac',
  ].join("\n"));
  ...
  fake("node", [
    ...
    '  if [[ "$code" == *JSON.stringify* && "$code" == *previousSlot=s.activeSlot* ]]; then [[ "$content" =~ \"activeSlot\":\"([^\"]+)\" ]] && active_slot="${BASH_REMATCH[1]}"; [[ "$content" =~ \"activeImage\":\"([^\"]+)\" ]] && active_image="${BASH_REMATCH[1]}"; printf "{\\\"activeSlot\\\":\\\"%s\\\",\\\"activeImage\\\":\\\"%s\\\",\\\"previousSlot\\\":\\\"%s\\\",\\\"previousImage\\\":\\\"%s\\\",\\\"postgresMode\\\":\\\"neon\\\",\\\"redisMode\\\":\\\"upstash\\\"}" "${4}" "${5}" "$active_slot" "$active_image"; exit 0; fi',
    '  if [[ "$code" == *JSON.stringify* ]]; then [[ "$content" =~ \"activeSlot\":\"([^\"]+)\" ]] && active_slot="${BASH_REMATCH[1]}"; [[ "$content" =~ \"activeImage\":\"([^\"]+)\" ]] && active_image="${BASH_REMATCH[1]}"; [[ "$content" =~ \"previousSlot\":\"([^\"]+)\" ]] && previous_slot="${BASH_REMATCH[1]}"; [[ "$content" =~ \"previousImage\":\"([^\"]+)\" ]] && previous_image="${BASH_REMATCH[1]}"; printf "{\\\"activeSlot\\\":\\\"%s\\\",\\\"activeImage\\\":\\\"%s\\\",\\\"previousSlot\\\":\\\"%s\\\",\\\"previousImage\\\":\\\"%s\\\",\\\"postgresMode\\\":\\\"neon\\\",\\\"redisMode\\\":\\\"upstash\\\"}" "$previous_slot" "$previous_image" "$active_slot" "$active_image"; exit 0; fi',
    '  if [[ "$code" == *activeSlot* ]]; then [[ "$content" =~ \"activeSlot\":\"([^\"]+)\" ]] && printf "%s" "${BASH_REMATCH[1]}"; exit 0; fi',
    '  if [[ "$code" == *previousSlot* ]]; then [[ "$content" =~ \"previousSlot\":\"([^\"]+)\" ]] && printf "%s" "${BASH_REMATCH[1]}"; exit 0; fi',
    '  if [[ "$code" == *previousImage* ]]; then [[ "$content" =~ \"previousImage\":\"([^\"]+)\" ]] && printf "%s" "${BASH_REMATCH[1]}"; exit 0; fi',
    '  if [[ "$code" == *activeImage* ]]; then [[ "$content" =~ \"activeImage\":\"([^\"]+)\" ]] && printf "%s" "${BASH_REMATCH[1]}"; exit 0; fi',
    '  if [[ "$code" == *serverName* ]]; then printf "www.cloudflare.com"; exit 0; fi',
    '  if [[ "$code" == *target* ]]; then printf "host.docker.internal:8443"; exit 0; fi',
    'fi',
    'case "$*" in',
