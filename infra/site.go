...
type Site struct {
	...
	FinalBarrier *local.Command
	Status       pulumi.MapOutput
}
...
func marshalRuntimeSecrets(secrets SiteSecrets) string {
	...
	}{secrets.AdminPassword, secrets.JWTSecret, secrets.TOTPEncryptionKey, secrets.Database, secrets.Redis}
...
func marshalAppEnv(appEnv *map[string]string) string {
	if appEnv == nil {
		return "{}"
	}
	encoded, err := json.Marshal(*appEnv)
	if err != nil {
		panic(err)
	}
	return string(encoded)
...
func postgresRuntimePassword(spec SiteSpec, secrets SiteSecrets, databasePassword interface{}) interface{} {
	if spec.Database.Mode == "docker" {
		return secrets.Database.Password
	}
...
func DeploySite(ctx *pulumi.Context, siteID string, spec SiteSpec, secrets SiteSecrets, appEnv pulumi.StringOutput, layout SiteLayout, edge *Edge, preflight, previousBarrier pulumi.Resource, checksum, configuredSiteIDs string) (*Site, error) {
	...
	if secrets.AppEnv != nil {
		environment["APP_ENV_CONFIGURED"] = pulumi.String("true")
	}
	reconcile, err := newSiteCommand(ctx, layout, "site-"+siteID+"-reconcile", "infra-reconcile", "bash scripts/reconcile-site.sh", environment, reconcileTriggers, site, dependencies...)
	if err != nil {
		return nil, err
	}
	releaseEnvironment := siteEnvironment(siteID, spec, layout, runtime, appEnv, configuredSiteIDs, edge.Spec.OriginIP)
	if secrets.AppEnv != nil {
		releaseEnvironment["APP_ENV_CONFIGURED"] = pulumi.String("true")
	}
	releaseEnvironment["SUB2API_IMAGE"] = pulumi.String(spec.Image)
	release, err := newSiteCommand(ctx, layout, "site-"+siteID+"-release", "application-release", "bash scripts/application-release.sh", releaseEnvironment, BuildSiteReleaseTriggers(siteID, spec.Image), site, reconcile, preflight)
	if err != nil {
		return nil, err
	}
	readiness, err := newSiteCommand(ctx, layout, "site-"+siteID+"-strict-public-readiness", "post-strict-public-readiness", `bash scripts/probe-origin.sh "$DOMAIN" "$APP_PROBE_PATH"`, environment, reconcileTriggers, site, release, preflight)
	if err != nil {
		return nil, err
	}
	rollback, err := newCommand(ctx, "site-"+siteID+"-rollback-preparation", "bash -c 'exit 0'", environment, reconcileTriggers, site, readiness, preflight)
	if err != nil {
		return nil, err
	}
