...
func TestHostDesiredStateDigestTracksOrdinarySideEffectsOnly(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if base == imageDigest { t.Fatal("host desired-state digest ignores Site image") }
	edgeChanged := host
	edgeChanged.Edge.ACMEEmail = "new-ops@example.com"
	edgeDigest, err := hostDesiredStateDigest(edgeChanged, layouts)
	if err != nil { t.Fatal(err) }
	if base == edgeDigest { t.Fatal("host desired-state digest ignores Edge config") }
	siteChanged := host
...
func TestChecksumBoundariesKeepOwnerSpecificFilesIsolated(t *testing.T) {
	...
	for _, required := range []string{"compose/edge.yml", "scripts/edge-compose-common.sh", "scripts/reconcile-edge.sh", "scripts/render-edge-config.ts", "scripts/render-runtime-env.ts", "traefik/dynamic/sing-box.yml"} { if !edgePaths[required] { t.Fatalf("edge checksum omits %q", required) } }
	sitePaths := map[string]bool{}
	for _, path := range siteChecksumPaths { sitePaths[path] = true }
	for _, required := range []string{"compose/site.yml", "compose/upstream.yml", "scripts/site-compose-common.sh", "scripts/read-runtime-env.cjs", "scripts/reconcile-site.sh", "scripts/bootstrap-site.sh", "scripts/application-release.sh", "scripts/switch-slot.sh", "scripts/rollback-slot.sh", "scripts/render-site-route.ts", "scripts/render-runtime-env.ts", "scripts/deployment-mode.ts", "scripts/write-deploy-state.ts", "scripts/write-bootstrap-marker.ts", "src/deployment-preflight.ts", "traefik/dynamic/site.yml"} { if !sitePaths[required] { t.Fatalf("site checksum omits %q", required) } }
	for _, edgeOnly := range []string{"compose/edge.yml", "scripts/edge-compose-common.sh", "scripts/reconcile-edge.sh", "scripts/render-edge-config.ts", "traefik/dynamic/sing-box.yml"} { if sitePaths[edgeOnly] { t.Fatalf("Site checksum includes Edge-only path %q", edgeOnly) } }
	for _, siteOnly := range []string{"compose/site.yml", "compose/upstream.yml", "scripts/site-compose-common.sh", "scripts/read-runtime-env.cjs", "scripts/reconcile-site.sh", "scripts/bootstrap-site.sh", "scripts/application-release.sh", "scripts/switch-slot.sh", "scripts/rollback-slot.sh", "scripts/render-site-route.ts", "scripts/deployment-mode.ts", "scripts/write-deploy-state.ts", "scripts/write-bootstrap-marker.ts", "src/deployment-preflight.ts", "traefik/dynamic/site.yml"} { if edgePaths[siteOnly] { t.Fatalf("Edge checksum includes Site-only path %q", siteOnly) } }
	hostPaths := map[string]bool{}
	for _, path := range hostChecksumPaths { hostPaths[path] = true; if edgePaths[path] || sitePaths[path] { t.Fatalf("host checksum path %q overlaps Edge or Site", path) } }
	for _, required := range []string{"scripts/host-preflight.ts", "scripts/finalize-host-state.sh", "scripts/write-host-state.ts"} { if !hostPaths[required] { t.Fatalf("host checksum omits %q", required) } }
}
