...
type Edge struct {
	...
	Provider  *cloudflareresource.Provider
...
func DeployEdge(ctx *pulumi.Context, spec EdgeSpec, apiToken pulumi.StringInput, checksum string, preflight pulumi.Resource, legacyCode2 bool) (*Edge, error) {
	...
	provider, err := createCloudflareProvider(ctx, edge, preflight, apiToken, legacyCode2)
	if err != nil {
		return nil, err
	}
	ssl, err := createStrictSSLSetting(ctx, edge, preflight, provider, spec.CloudflareZoneID, legacyCode2)
	if err != nil {
		return nil, err
	}
	singBox, err := json.Marshal(spec.SingBox)
	if err != nil {
		return nil, err
	}
