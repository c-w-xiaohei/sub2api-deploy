...
func siteRedisInputs(ctx *pulumi.Context, site, preflight pulumi.Resource, siteID string, spec SiteSpec, secrets SiteSecrets) (RedisConnectionInputs, error) {
	...
	if spec.Redis.ResourceMode == "create" {
		provider, err := upstash.NewProvider(ctx, "site-"+siteID+"-upstash", &upstash.ProviderArgs{ApiKey: pulumi.ToSecret(pulumi.String(secrets.Redis.APIKey)).(pulumi.StringOutput)}, pulumi.Parent(site), pulumi.DependsOn([]pulumi.Resource{preflight}), pulumi.Version("0.5.0")); if err != nil { return RedisConnectionInputs{}, err }
		// Retirement must explicitly unprotect this persistent database first.
		database, err := upstash.NewRedisDatabase(ctx, "site-"+siteID+"-upstash-redis", &upstash.RedisDatabaseArgs{DatabaseName: pulumi.String(ManagedUpstashDatabaseName(spec.ResourcePrefix, "")), Region: pulumi.String(spec.Redis.Region), Tls: pulumi.BoolPtr(true)}, pulumi.Parent(site), pulumi.Provider(provider), pulumi.DependsOn([]pulumi.Resource{preflight}), pulumi.Version("0.5.0"), pulumi.Protect(true), pulumi.RetainOnDelete(true)); if err != nil { return RedisConnectionInputs{}, err }
