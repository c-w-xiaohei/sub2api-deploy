...
sub2api-deploy runtime deployment-mode check "$SITE_DEPLOY_STATE_PATH" "$POSTGRES_MODE" "$REDIS_MODE"
active_slot="$(sub2api-deploy runtime read-state "$SITE_DEPLOY_STATE_PATH" activeSlot)"
SUB2API_IMAGE="$(sub2api-deploy runtime read-state "$SITE_DEPLOY_STATE_PATH" activeImage)"
mkdir -p "$BLUE_DATA_PATH" "$GREEN_DATA_PATH"
printf '%s' "$RUNTIME_JSON" | sub2api-deploy runtime dotenv write "$SITE_RUNTIME_ENV_PATH" --slot="$active_slot" --slot-data-dir="$active_slot"
...
sub2api-deploy runtime route write traefik/dynamic/site.yml "$SITE_ROUTE_PATH" "$SITE_ID" "$DOMAIN" "$active_slot" "$alias"
