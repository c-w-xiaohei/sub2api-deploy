...
previous_slot="$(node -e 'const s=require(process.argv[1]); if (!s.previousSlot || !s.previousImage) process.exit(1); process.stdout.write(s.previousSlot)' "$state_file")"
previous_image="$(node -e 'const s=require(process.argv[1]); process.stdout.write(s.previousImage)' "$state_file")"
...
state_json="$(node -e 'const s=require(process.argv[1]); const old={activeSlot:s.activeSlot,activeImage:s.activeImage}; s.activeSlot=s.previousSlot; s.activeImage=s.previousImage; s.previousSlot=old.activeSlot; s.previousImage=old.activeImage; process.stdout.write(JSON.stringify(s))' "$state_file")"
