...
runtime_root="$(dirname "$host_state")"; legacy_state="$runtime_root/deploy-state.json"; legacy_acme="$runtime_root/acme.json"
...
require_apply_inputs() { : "${TRAEFIK_IMAGE:?TRAEFIK_IMAGE is required}" "${CLOUDFLARE_API_TOKEN:?CLOUDFLARE_API_TOKEN is required}" "${ACME_EMAIL:?ACME_EMAIL is required}" "${SING_BOX_SERVER_NAME:?SING_BOX_SERVER_NAME is required}" "${SING_BOX_TARGET:?SING_BOX_TARGET is required}" "${DOMAIN:?DOMAIN is required}" "${ORIGIN_IP:?ORIGIN_IP is required}" "${APP_PROBE_PATH:?APP_PROBE_PATH is required}" "${SING_BOX_VERIFY_COMMAND:?SING_BOX_VERIFY_COMMAND is required}"; }
slot() { node -e 'const x=require(process.argv[1]); if(!["blue","green"].includes(x.activeSlot)) process.exit(1); process.stdout.write(x.activeSlot)' "$legacy_state"; }
...
read_journal() {
  ...
  for key in "${!allowed[@]}"; do [[ -v "j[$key]" ]] || { printf 'journal is incomplete\n' >&2; exit 1; }; done
  [[ "${j[version]}" == 1 && "${j[environment]}" == "$environment" && "${j[site]}" == code2 && "${j[host_state]}" == "$host_state" && "${j[legacy_project]}" == sub2api && "${j[edge_project]}" == sub2api-edge && "${j[edge_root]}" == "$edge_root" && "${j[route]}" == "$route" && "${j[route_backup]}" == "$route_backup" && "${j[edge_network]}" == sub2api-edge && "${j[edge_container_preexisting]}" == false ]] || { printf 'journal does not match this host\n' >&2; exit 1; }
