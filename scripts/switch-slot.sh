...
mkdir -p "$SITE_RUNTIME_ROOT/data/${active_slot}" "$SITE_RUNTIME_ROOT/data/${inactive_slot}"
if [[ -f "$SITE_RUNTIME_ROOT/data/${active_slot}/config.yaml" ]]; then cp -p "$SITE_RUNTIME_ROOT/data/${active_slot}/config.yaml" "$SITE_RUNTIME_ROOT/data/${inactive_slot}/config.yaml"; fi
if [[ -f "$SITE_RUNTIME_ROOT/data/${active_slot}/.installed" ]]; then cp -p "$SITE_RUNTIME_ROOT/data/${active_slot}/.installed" "$SITE_RUNTIME_ROOT/data/${inactive_slot}/.installed"; fi

export SUB2API_IMAGE="$image" SLOT="$inactive_slot" SLOT_DATA_DIR="$inactive_slot" AUTO_SETUP=false
"${SITE_COMPOSE[@]}" up -d --wait --wait-timeout 300 "sub2api-${inactive_slot}"
...
for ((attempt = 1; attempt <= probe_retries; attempt++)); do
  if "${SITE_COMPOSE[@]}" exec -T "sub2api-${inactive_slot}" wget -q -T 10 -O /dev/null "$internal_probe_url"; then break; fi
...
previous_route="$SITE_ROUTE_PATH.before-switch"
had_route=false
