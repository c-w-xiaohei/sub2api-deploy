...
- 待产品/owner 确认的阻塞项：官方 Neon Provider package/resource schema/create outputs/`protect`/secret-unknown projection、跨 Host allowlist 合同、MicroSocks/Connector 执行合同。见第 11 节。
...
已完成的目标源码事实包括：严格 YAML/SOPS 配置解析与 SSH alias 校验；Host contract/revision、单帧严格协议、系统 OpenSSH 固定 argv 与 artifact bootstrap；唯一 `Host` Provider schema 及主要 lifecycle；远端每 Host 状态、writer lock、journal、read-only inspect、blue/green、本地 data/proxy、preserve-data retire；以及 Environment Program 对 Host、Cloudflare、Upstash 的初步注册。部分完成的事实包括：CLI 仍只公开 `validate`，虽已有受限 Pulumi 参数解析、staged stack 和 fd 3 approval 代码；Program 仅支持 DNS Cloudflare，拒绝 Neon、SingBox、outbound proxy 与跨 Host Docker data；runtime 明确拒绝 MicroSocks/Connectors。主要缺口是将这些切片串成受支持的产品路径、补齐官方 Neon 模型、冻结跨 Host 网络和 proxy/connector 合同，并制作/发布新的日常 release。
...
| 薄 CLI | `cmd/sub2api-deploy/**` | 已有 `validate`、SOPS decrypt、SSH 预检、受限 Pulumi operation parse、staged stack、TTY approval。公开日常 `pulumi` 路径尚未完整接线。 |
...
当前 `internal/program` 的事实是：Cloudflare DNS 和 Upstash 已有初步 direct registration；Cloudflare 只接受 DNS 模式；managed Neon、SingBox、outbound proxy、跨 Host Docker PostgreSQL/Redis 都在 preflight 显式拒绝。产品 Neon gap 只包括官方 Provider package/resource schema、create outputs、`protect` 和 secret/unknown 投影；它不以既有 physical ID 的 import/no-replace 证据为前置条件。后者只属于附录迁移。TR-PROG-* 的规范性完整定义见第 10 节。
...
| Import | program-first、只读 state construction；证据不足失败，不靠 apply 猜测接管。 | 目标行为；实现必须继续保持 read-only。 |

TR-LC-* 的规范性完整定义见第 10 节 Requirement Index；当前 Provider 没有成功的 Import 实现，因而 Import 是 001 产品本体的明确 product gap。普通新 Environment 的 Create 不依赖 Import；但 program-first、只读的 Host Import 本身仍必须在 001 实现，不能写作“部分完成”。
...
`apps.<app>.servers: []` 是唯一 Pulumi-visible maintenance 表达，前提是该 App 已从 public access 摘除。它保留 App 定义和 data links，但从所有原 Host 完整目标中移除该 App runtime/writers、保留数据；恢复时稳定排序第一台 Host ready 后再启动其余 Host，最后恢复 public access。TR-MAINT-* 与 TR-ORDER-* 的规范性完整定义见第 10 节；跨 Host 数据 allowlist 的具体执行合同仍未冻结，见阻塞项。
...
新 release 必须包含薄 CLI、Environment Program、Host Provider、`sub2api-host` 和其可验证 artifact bundle；控制机而非服务器运行 Pulumi/Provider/SOPS/cloud credentials。release 接线、CLI/Program 主链、CI artifact verification 与对应 README 是当前产品本体实施内容；它们尚未完成，因而不能把 current source binary 或任何测试切片表述为已发布 release。现有生产实例的 legacy writer 退出属于附录迁移，不是新 release 主链实现的前置条件。CI 才能提供构建与发布产物证据；本规格不声称 CI 已通过。
...
| Environment Program | 补齐 | `cmd/sub2api-environment/**`, `internal/program/**` | 官方 Providers、每 server 一个 Host、graph/order | Neon/allowlist blocker 关闭前不可宣称完整。 |
...
- 开放风险：Neon、cross-host allowlist、MicroSocks/Connector 三项尚未能以现有实现或 source 自动推导，必须先作 owner decision，不能用更大抽象掩盖缺口。
...
| MUST NOT | 将未冻结的 Neon、allowlist、MicroSocks/Connector 细节伪造为既定合同。 |
...
| AC-02 | 完整 Host lifecycle，Create install+reconcile，Read/Import read-only。 | 部分：Create/Read 等代码切片存在；成功的 Import 仍是当前 product gap。 |
| AC-03 | OpenSSH 与 protocol 安全。 | 部分：源码/测试存在，未声称 CI/release evidence。 |
| AC-04 | unknown-result 单副作用恢复。 | 部分：journal/runtime 代码存在。 |
| AC-05 | blue/green 与跨 Host failure stop。 | 部分：本机 blue/green 有实现；跨 Host allowlist/order 未闭合。 |
...
验证必须覆盖 `TS-P0-*` 的 Program、lifecycle、SSH/protocol、recovery、blue/green、order、maintenance、data approval、Delete、secret 分层；migration P0/P1 只在后续迁移授权时启用。允许静态/单元/局部 rehearsal 的实现阶段 gate 由 Test Spec 定义；本轮文档工作没有运行构建、测试、vet、Docker、SSH、Pulumi、release 或动态验证。本文代码事实来自本轮可读工作树，不主张具有当前 exact-SHA 的动态执行证据。
...
| 跨 Host PostgreSQL/Redis allowlist 的合法目标、执行顺序、readiness 与撤销 | 产品/infra owner | 允许跨 Host data 前 | BLOCKED；当前 Program 拒绝 cross-host Docker data。 |
