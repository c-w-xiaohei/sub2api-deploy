...
Nix schema 2 is the current release boundary. Descriptor tests must prove one
exact candidate plus `hostPayload` records for both supported systems, and Host
composition tests must prove `bin/sub2api-host` and release metadata in the
production profile. Provider tests must prove probe and fixed-profile
invocation only: no upload, bootstrap receiver, upgrade, or
`/usr/local/libexec` mutation. A descriptor must be installed and activated
before Host preview/up. These changes supersede older bootstrap lifecycle test
descriptions; Host state, reconcile, removal, recovery, and Read/Import tests
remain applicable.
...
当前源码和测试仅是 `candidate-unverified` / `implementation-present-evidence-pending`：Engine Graph、Provider Runtime、Provider Import 和 target release harness/workflow 已在当前 dirty checkout，但没有精确远端 SHA 的 GHA run/artifact 时，任何源码测试都不是 `covered` 或 `passed`。`product-gap` 表示产品能力尚未提供；`harness-gap` 表示能力/局部机制存在但所需验证边界未闭合；`blocked-decision` 表示不能由当前 source 或字段自动推导的 owner 决策。
...
| --- | --- | --- | --- |
| `internal/program/program_test.go` | two Host registrations, Cloudflare/Upstash inputs, protect, secret scope, deterministic map projection, maintenance suppression | candidate-unverified / implementation-present-evidence-pending | Engine/local-backend harness exists; Task4 cross-Host allowlist/failure-stop/publication coverage remains a gap |
| `internal/hostprovider/provider_test.go`, `lifecycle_test.go` | property class purity, lifecycle recording transport, Create checkpointing, Read ID preservation, approval/Delete local behavior, successful read-only program-first Import | candidate-unverified / implementation-present-evidence-pending | Provider-process-to-runtime connection has a current-checkout harness; no exact-SHA evidence |
...
| `cmd/sub2api-host/main_test.go` | one-frame process behavior, process exit, reconcile result, FD3 attestation | candidate-unverified / implementation-present-evidence-pending | CI-only invocation with temp runtime, no production root changes |
...
GHA gate labels identify required evidence. Engine Graph, Provider Runtime, Provider Import, and target release jobs/harnesses exist in the current dirty checkout, but no matching exact 40-hex remote SHA, run URL, named result, and artifact is present here; no row is `passed`.
...
| MX-ALLOWLIST-01 | TR-PROG-06, TR-ORDER-02/03 | Decision+Pairwise | cross-Host data Host/App Host; allow absent/present; ready/fail | staged update/delete graph | allow-source -> App readiness -> publication and inverse detach order | absent allowlist/failure => App/publication 0 | real Engine graph + test publication Provider; candidate Provider/Host + two-Host live nft/Docker fixture | Program/Infra | `TestEngineGraphCrossHostDataAdmissionOrderingAndFailureStop`, `TestEngineGraphCrossHostDataRemovalIsReverseStaged`, `TestEngineGraphTraceArtifactIsSanitizedJSONL`, `TestProviderRuntimeCrossHostDataAdmissionLive` | `GHA@REMOTE_SHA:engine-graph`, `GHA@REMOTE_SHA:provider-runtime` | implementation-present/evidence-pending; exact-SHA execution pending |
...
| MX-HOST-PROCESS-01 (new) | TR-INV-04, TR-PROTO-01..04 | EP+State+Negative | inspect/reconcile frame; second frame | invoke in-process serve or spawn test binary | existing inspect/reconcile process tests and `TestInstallAttestProcessUsesOnlyFD3` cover real subprocess behavior | second/write-invalid frame rejected; subprocess exits; no persistent process | in-process pipes versus real test subprocess + pipe/FD3, not remote SSH | Host-process | named existing symbols; planned: `TestHostProcessWithTestOnlyRuntimeHelper` | planned `GHA@REMOTE_SHA:host-process` | candidate-unverified / implementation-present-evidence-pending |

### 4.3 Runtime / Import / Recovery / Data

| MX ID | source | technique | prestate | stimulus | observable | no-side-effect | real/mock | owner | test symbol | GHA gate | status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MX-LC-01 (`TS-P0-CREATE-01`) | TR-LC-CREATE-* | State | activated matching Nix profile | Provider Create | lifecycle test asserts probe/reconcile/inspect/checkpoint ordering | profile mismatch/ownership stops before reconcile | Provider recording transport | Provider | `TestLifecycleCreateUsesProbeHostReconcileInspectAndCheckpoints`, `TestLifecycleCreateGuardsFailBeforeArtifactOrTransport` | planned `GHA@REMOTE_SHA:provider-lifecycle` | candidate-unverified / implementation-present-evidence-pending |
...
| MX-IMPORT-02 (`TS-P0-IMPORT-01`, success) | TR-INV-06, TR-LC-IMPORT-01..05 | Decision+State | Program registered complete Host; matching machine/ownership/runtime/path/data proof | Import then preview | program-first state construction then no-op/accepted safe diff | inspect only; no install/reconcile/ownership write/render/runtime mutation | module seam + Engine local backend | Provider/Integration | `TestImportBuildsReadOnlyStateFromVerifiedObservation`; `TestEngineImportPreviewIsNoOpOrAcceptedDiff` | `GHA@REMOTE_SHA:provider-import` | implementation-present/evidence-pending; tests and CI job exist, exact-SHA evidence pending |
...
| MX-SEC-01 (`TS-P0-SEC-01`) | TR-SEC-04/05, TR-OBS-03 | Negative | distinct per-layer canaries | Program/SSH/runtime execution | existing symbols only cover their named local diagnostic/property boundary | planned cross-layer scan must find zero in argv/output/log/stderr/journal/non-target Host | module seams; planned layered artifacts | Security | `TestRunDoesNotExposeStderrCanary`; planned: `TestLayeredSecretCanaryContainment` | planned `GHA@REMOTE_SHA:secret-boundaries` | candidate-unverified / implementation-present-evidence-pending |

## 5. Current Exact-SHA CI Jobs

`verify`、`host-controller`、Engine Graph、Provider SSH、Provider Runtime、Provider Import 和 Target Release jobs 已在当前 dirty checkout 的 CI workflow 中实现。它们尚未为当前 cross-Host candidate 收集 passing exact-SHA run/artifact evidence。动态证据只能来自 GitHub Actions 对精确 40 位远端 SHA 的 run；本地仅允许静态阅读、搜索和本文档编辑。`MX-ALLOWLIST-01` 的 Task4 dedicated Engine、two-Host live Provider Runtime 和 candidate-consumption gates 已实现但尚未执行；其他 Test Spec 历史或 planned harness/gate gaps 保留各自状态，并未因此关闭。

| Current job or remaining Task4 gate | Required evidence | Cannot be substituted by |
| --- | --- | --- |
| `verify`, `host-controller`, Engine Graph, Provider SSH, Provider Runtime, Provider Import, Target Release | exact SHA/run URL, named results, and each job's required sanitized trace or candidate-consumer artifact | branch/latest/cache/local claim or a source-only test |
| Task4 dedicated `MX-ALLOWLIST-01` allowlist/live gates | exact-SHA Engine/runtime and live nft/PostgreSQL/Redis evidence for cross-Host admission, ordering, failure stop, and removal | generic existing job coverage or source-only tests |
| future owner-contract or migration gates | decided feature artifacts or authorized appendix-only sanitized migration rehearsal | ordinary 001 product evidence |

P0 enforcement contract: this Test Spec is the coverage inventory; CI MUST NOT create a selector registry or auxiliary manifest. Each current job owns fixed commands for its implemented symbols and emits `go test -json` or the runner's equivalent JSON event stream where applicable. The job fails when a required named test has no terminal `pass`, emits `skip`, is absent/not-run, belongs to a different SHA, or lacks the trace artifact required by its matrix row. Final P0 acceptance requires every applicable P0 MX to have collected exact-SHA `passed` evidence; local-only, mock-only, branch-only or cached-only results are not substitutes.

## 6. Requirement -> MX -> Symbol -> Planned Gate Mapping

| Requirement | MX | Existing narrow symbol / planned symbol | Current job or remaining gate |
| --- | --- | --- | --- |
...
| --- | --- | --- |
| P0 final acceptance | no-skip at final acceptance | candidate source evidence and current CI jobs do not yet close P0 | controller collects exact-SHA `passed` evidence per applicable row |
| Current CI jobs | implementation-present/evidence-pending | Engine Graph, Provider Runtime, Provider Import, and target release jobs/harnesses exist in the current dirty checkout; no exact-SHA run/artifact is present | workflow owner obtains and records exact-SHA evidence before final acceptance |
...
| cross-Host allowlist | implementation-present/evidence-pending | Tasks 1-3 derive legality, ownership, sequence, and safe removal; dedicated Engine and two-Host candidate/live Provider Runtime gates and workflow wiring exist | collect exact-SHA Engine/runtime and live nft/PostgreSQL/Redis evidence before acceptance |
...
| successful Import | implementation-present/evidence-pending | module and Engine success tests plus Provider Import CI job exist in the current dirty checkout | collect matching exact-SHA run/artifact evidence |
| public CLI subprocess wiring | implementation-present/evidence-pending + harness-gap | public command exists; its released subprocess/PTY boundary lacks exact-SHA CI evidence | CLI owner adds the planned CI-only public command evidence |
| target supported release | implementation-present/evidence-pending | target release workflow and isolated candidate-consumer harness exist in the current dirty checkout | collect exact-SHA candidate evidence; CI/release workflow owner只在最终全部适用P0 gates和candidate consumer通过后promotion/publish |
| Engine graph, Provider Runtime, Provider Import | implementation-present/evidence-pending | CI-only harnesses/jobs, including Task4 cross-Host allowlist and live nft/PostgreSQL/Redis source gates, exist in the current dirty checkout | collect exact-SHA evidence; source presence alone is not a passing result |
