...
func TestRegisterProjectsPaymentGatewaysToSelectedHostsAndPublishesSelectedHostDNS(t *testing.T) {
	...
	for _, placement := range placements {
		...
		for _, host := range []pulumi.MockResourceArgs{alpha, bravo, charlie} {
			if !containsURN(directDependencies(host), placement) {
				t.Fatalf("Host %q must depend on placement %q: %v", host.Name, placement.Name, directDependencies(host))
			}
		}
	}
...
func TestRegisterProjectsSameHostDockerDataOnlyToOwner(t *testing.T) {
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, sameHostDockerConfig(), dockerSecrets()); err != nil {
		t.Fatal(err)
	}
	assertNoCalls(t, mocks)
	...
	assertManagedApp(t, owner, "app", "app-postgres", postgresIdentity("docker:alpha:app-postgres", "app-postgres", 5432, "sub2api", ""), "app-redis", redisIdentity("docker:alpha:app-redis", "app-redis", 6379, "0"), "app-postgres-password-canary", "app-redis-password-canary")
	identity := object(t, dataLink(t, appTarget(t, owner.Inputs, "app"), "app-postgres"), "identity")
	if stringValue(t, property(identity, "tlsMode")) != "disable" || optionalString(t, property(identity, "tlsServerName")) != "" {
		t.Fatalf("same-Host PostgreSQL TLS = %v", identity)
	}
...
func TestRegisterProjectsExternalDataWithoutCloudResources(t *testing.T) {
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, externalConfig(), externalSecrets()); err != nil {
		t.Fatal(err)
	}
	assertNoCalls(t, mocks)
	for _, token := range []string{"upstash:index/redisDatabase:RedisDatabase", "neon:provider:Project"} {
...
func TestRegisterCreatesSeparateUpstashProvidersForDistinctServiceKeys(t *testing.T) {
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, twoUpstashConfig(), twoUpstashSecrets()); err != nil {
		t.Fatal(err)
	}
	...
	if len(providers) != 2 || len(databases) != 2 {
		t.Fatalf("Upstash providers/databases = %d/%d, want 2/2", len(providers), len(databases))
	}
	byDatabase := map[string]pulumi.MockResourceArgs{}
	for _, database := range databases {
		byDatabase[stringValue(t, property(database.Inputs, "databaseName"))] = database
	}
...
func TestRegisterMaintenanceKeepsManagedAndLocalData(t *testing.T) {
	for _, tc := range []struct {
		...
		managed               bool
	}{{"managed", managedMaintenanceConfig(), managedNoPublicSecrets(), true}, {"docker", dockerMaintenanceConfig(), dockerSecrets(), false}} {
		t.Run(tc.name, func(t *testing.T) {
			mocks := &recordingMocks{}
			if err := runRegister(t, mocks, pinnedRelease, tc.config, tc.secrets); err != nil {
				t.Fatal(err)
			}
			...
			if tc.managed {
				database := managedUpstash(t, mocks.resources)
				if !database.RegisterRPC.GetProtect() || !database.RegisterRPC.GetRetainOnDelete() {
					t.Fatal("maintenance must retain protected managed Redis")
				}
			} else {
				assertLocalServices(t, hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "alpha"))
			}
		})
	}
}
...
func TestRegisterPreservesComputedUpstashOutputs(t *testing.T) {
	...
	for _, output := range []struct {
		...
		{"Host target", pulumi.ToOutput(hostTarget(validated.Config, secrets, pinnedRelease, "alpha", managed)), false},
...
func TestValidateDockerDataClientsAggregatesMultipleRemoteConsumers(t *testing.T) {
	port := 5432
	config := environment.Config{Servers: map[string]environment.Server{
		"data": {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.1"}}},
		"one":  {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.2"}}},
		"two":  {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.3"}}},
	}, Postgres: map[string]environment.Postgres{"db": {Type: "docker", Server: "data", Port: &port}}, Redis: map[string]environment.Redis{}, Apps: map[string]environment.App{
		"one": {Servers: []string{"one"}, Postgres: environment.AppPostgres{Name: "db", Database: "app"}},
		"two": {Servers: []string{"two"}, Postgres: environment.AppPostgres{Name: "db", Database: "app"}},
	}}
	secrets := environment.Secrets{Apps: map[string]environment.AppSecrets{"one": {Postgres: &environment.AppPostgresSecrets{Username: "app", Password: "password"}}, "two": {Postgres: &environment.AppPostgresSecrets{Username: "app", Password: "password"}}}}
	if err := validateDockerDataClients(config, secrets); err != nil {
		t.Fatalf("same service remote consumers collided: %v", err)
	}
}

func TestRegisterCrossHostDockerProjectsRemotePostgresAndRedis(t *testing.T) {
	for _, kind := range []string{"postgres", "redis"} {
		t.Run(kind, func(t *testing.T) {
			mocks := &recordingMocks{}
			if err := runRegister(t, mocks, pinnedRelease, crossHostDockerConfig(kind), crossHostDockerSecrets(kind)); err != nil {
				t.Fatal(err)
			}
			owner, app := hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "alpha"), hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "bravo")
			identity := object(t, dataLink(t, appTarget(t, app.Inputs, "app"), map[string]string{"postgres": "app-postgres", "redis": "app-redis"}[kind]), "identity")
			if stringValue(t, property(identity, "endpoint")) != "10.0.0.11" || stringValue(t, property(identity, "providerId")) != "docker:alpha:app-"+kind {
				t.Fatalf("remote %s identity = %v", kind, identity)
			}
			target, _ := decodeKnownHost(t, owner)
			if len(target.DataServices) != 1 || len(target.DataServices[0].Bindings) != 1 || target.DataServices[0].Bindings[0].Address != "10.0.0.11" || len(target.DataServices[0].Bindings[0].AllowedSources) != 1 || target.DataServices[0].Bindings[0].AllowedSources[0] != "10.0.0.12" {
				t.Fatalf("owner bindings = %#v", target.DataServices)
			}
			admin := map[string]string{"postgres": postgresAdmin, "redis": redisAdmin}[kind]
			assertHostSecrets(t, owner, []string{admin, adminPassword}, []string{})
			assertHostSecrets(t, app, []string{}, []string{admin, adminPassword})
		})
	}
}

func TestValidateDockerDataClientsAcceptsIdenticalDuplicatePrincipal(t *testing.T) {
	port := 5432
	config := environment.Config{Servers: map[string]environment.Server{
		"data": {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.1"}}},
		"one":  {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.2"}}},
	...
	secrets := environment.Secrets{Apps: map[string]environment.AppSecrets{"one": {Postgres: &environment.AppPostgresSecrets{Username: "app", Password: "password"}}, "two": {Postgres: &environment.AppPostgresSecrets{Username: "app", Password: "password"}}}}
	if err := validateDockerDataClients(config, secrets); err != nil {
		t.Fatalf("identical principal tuple rejected: %v", err)
	}
}

func TestValidateDockerDataClientsPostgresProvisioningContract(t *testing.T) {
	for _, tc := range []struct {
		name, appID, username, database, password string
		wantErr                                   bool
	}{
		{"reserved username", "app", "s2h_client", "app", "safe", true},
		{"invalid identifier", "app", "app-user", "app", "safe", true},
		{"system database", "app", "app", "postgres", "safe", true},
		{"unsafe DSN password", "app", "app", "app", "bad\npassword", true},
		{"valid hyphenated App ID and punctuation", "app-id", "app", "app", "$;=\"quoted'", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			port := 5432
			config := environment.Config{Servers: map[string]environment.Server{"data": {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.1"}}}, "app": {Addresses: &environment.Addresses{Internal: &environment.AddressSet{IPv4: "10.0.0.2"}}}}, Postgres: map[string]environment.Postgres{"db": {Type: "docker", Server: "data", Port: &port}}, Redis: map[string]environment.Redis{}, Apps: map[string]environment.App{tc.appID: {Servers: []string{"app"}, Postgres: environment.AppPostgres{Name: "db", Database: tc.database}}}}
			secrets := environment.Secrets{Apps: map[string]environment.AppSecrets{tc.appID: {Postgres: &environment.AppPostgresSecrets{Username: tc.username, Password: tc.password}}}}
			err := validateDockerDataClients(config, secrets)
			if (err != nil) != tc.wantErr {
				t.Fatalf("validateDockerDataClients error = %v, want error=%t", err, tc.wantErr)
			}
			if tc.wantErr && err == nil {
				t.Fatal("invalid preflight would permit Pulumi registration")
			}
		})
	}
}

func TestRegisterCrossHostDockerIPv4PreferenceIPv6FallbackAndTLS(t *testing.T) {
	config := crossHostDockerConfig("postgres")
	config = strings.Replace(config, "ipv4: 10.0.0.11", "ipv4: 10.0.0.11\n        ipv6: 2001:db8::11", 1)
	config = strings.Replace(config, "ipv4: 10.0.0.12", "ipv4: 10.0.0.12\n        ipv6: 2001:db8::12", 1)
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, config, crossHostDockerSecrets("postgres")); err != nil {
		t.Fatal(err)
	}
	app := hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "bravo")
	identity := object(t, dataLink(t, appTarget(t, app.Inputs, "app"), "app-postgres"), "identity")
	if stringValue(t, property(identity, "endpoint")) != "10.0.0.11" || stringValue(t, property(identity, "tlsMode")) != "disable" {
		t.Fatalf("IPv4/TLS projection = %v", identity)
	}
	config = strings.Replace(config, "        ipv4: 10.0.0.11\n", "", 1)
	config = strings.Replace(config, "        ipv4: 10.0.0.12\n", "", 1)
	mocks = &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, config, crossHostDockerSecrets("postgres")); err != nil {
		t.Fatal(err)
	}
	app = hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "bravo")
	identity = object(t, dataLink(t, appTarget(t, app.Inputs, "app"), "app-postgres"), "identity")
	if stringValue(t, property(identity, "endpoint")) != "2001:db8::11" {
		t.Fatalf("IPv6 fallback = %v", identity)
	}
}

func TestRegisterCrossHostDockerAggregatesSourcesAndUsesDataDAGPlacement(t *testing.T) {
	config := crossHostDockerConfig("postgres")
	config = strings.Replace(config, "postgres:\n", "  charlie:\n    sshAlias: charlie-ssh\n    addresses:\n      internal:\n        ipv4: 10.0.0.13\npostgres:\n", 1)
	config = strings.Replace(config, "apps:\n", "apps:\n  app-two:\n    hostname: app-two.example.test\n    image: ghcr.io/example/sub2api@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd\n    initialAdminEmail: ops@example.test\n    readinessPath: /ready\n    servers: [charlie]\n    postgres: {name: app-postgres, database: sub2api}\n    redis: {name: app-redis, database: 0}\n    publicAccess: {type: none}\n", 1)
	secrets := strings.Replace(crossHostDockerSecrets("postgres"), "apps:\n", "apps:\n  app-two:\n    initialAdminPassword: second-admin\n    jwtSecret: second-jwt\n    totpEncryptionKey: second-totp\n    postgres: {username: appuser, password: app-postgres-password-canary}\n    redis: {username: default, password: external-redis-password-canary}\n", 1)
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, config, secrets); err != nil {
		t.Fatal(err)
	}
	hosts := resourcesOfType(mocks.resources, hostresource.HostToken)
	owner := hostForServer(t, hosts, "alpha")
	target, _ := decodeKnownHost(t, owner)
	bindings := target.DataServices[0].Bindings
	if len(bindings) != 1 || bindings[0].Address != "10.0.0.11" || strings.Join(bindings[0].AllowedSources, ",") != "10.0.0.12,10.0.0.13" {
		t.Fatalf("aggregated bindings = %#v", bindings)
	}
	bravo, charlie := hostForServer(t, hosts, "bravo"), hostForServer(t, hosts, "charlie")
	assertHostDependsOn(t, bravo, owner)
	assertHostDependsOn(t, charlie, owner)
	if !optionalBool(t, property(appTarget(t, owner.Inputs, "app"), "initialBootstrap")) || optionalBool(t, property(appTarget(t, bravo.Inputs, "app"), "initialBootstrap")) {
		t.Fatal("data-DAG placement did not select the data owner as bootstrap Host")
	}
	assertHostSecrets(t, owner, []string{postgresAdmin}, []string{"second-admin"})
	assertHostSecrets(t, charlie, []string{"second-admin"}, []string{postgresAdmin})
}

func TestRegisterExternalDataProjectsCanonicalTLSModes(t *testing.T) {
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, externalConfig(), externalSecrets()); err != nil {
		t.Fatal(err)
	}
	app := hostForServer(t, resourcesOfType(mocks.resources, hostresource.HostToken), "alpha")
	for _, want := range []struct{ name, mode string }{{"app-postgres", "verify-full"}, {"app-redis", "require"}} {
		identity := object(t, dataLink(t, appTarget(t, app.Inputs, "app"), want.name), "identity")
		if stringValue(t, property(identity, "tlsMode")) != want.mode {
			t.Fatalf("%s TLS mode = %v", want.name, identity)
		}
	}
}

func TestRegisterCrossHostDockerPreflightRejectsCyclesSocketsAndPrincipals(t *testing.T) {
	for _, name := range []string{"cycle", "socket", "principal"} {
		t.Run(name, func(t *testing.T) {
			mocks := &recordingMocks{}
			config, secrets := crossHostDockerConfig("postgres"), crossHostDockerSecrets("postgres")
			switch name {
			case "cycle":
				config = strings.Replace(config, "type: external\n    host: redis.external.test\n    port: 6381\n    tls: true", "type: docker\n    server: bravo\n    port: 6379\n    persistence: true", 1)
				secrets = strings.Replace(secrets, "postgres:\n", "redis:\n  app-redis:\n    adminPassword: redis-admin-password-canary\npostgres:\n", 1)
			case "socket":
				config = strings.Replace(config, "type: external\n    host: redis.external.test\n    port: 6381\n    tls: true", "type: docker\n    server: alpha\n    port: 5432\n    persistence: true", 1)
				secrets = strings.Replace(secrets, "postgres:\n", "redis:\n  app-redis:\n    adminPassword: redis-admin-password-canary\npostgres:\n", 1)
			case "principal":
				config = strings.Replace(config, "apps:\n", "apps:\n  app-two:\n    hostname: app-two.example.test\n    image: ghcr.io/example/sub2api@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd\n    initialAdminEmail: ops@example.test\n    readinessPath: /ready\n    servers: [bravo]\n    postgres: {name: app-postgres, database: other}\n    redis: {name: app-redis, database: 0}\n    publicAccess: {type: none}\n", 1)
				secrets = strings.Replace(secrets, "apps:\n", "apps:\n  app-two:\n    initialAdminPassword: other\n    jwtSecret: other\n    totpEncryptionKey: other\n    postgres: {username: appuser, password: other}\n    redis: {username: default, password: redis}\n", 1)
			}
			if err := runRegister(t, mocks, pinnedRelease, config, secrets); err == nil || len(mocks.resources) != 0 {
				t.Fatalf("%s did not reject before resources: %v", name, err)
			}
		})
	}
}
...
func TestRegisterMaintenancePlacementKeepsHostsAndSuppressesPublication(t *testing.T) {
	mocks := &recordingMocks{}
	if err := runRegister(t, mocks, pinnedRelease, maintenanceConfig(), externalSecrets()); err != nil {
		t.Fatal(err)
	}
	assertNoCalls(t, mocks)
	assertCount(t, mocks.resources, hostresource.HostToken, 2)
...
func TestRegisterIsDeterministicAcrossYAMLMapOrder(t *testing.T) {
	first, second := &recordingMocks{}, &recordingMocks{}
	if err := runRegister(t, first, pinnedRelease, externalConfig(), externalSecrets()); err != nil {
		t.Fatal(err)
	}
	if err := runRegister(t, second, pinnedRelease, reorderedExternalConfig(), externalSecrets()); err != nil {
		t.Fatal(err)
	}
...
func postgresIdentity(providerID, endpoint string, port int, database, tls string) dataIdentity {
	return dataIdentity{providerID, endpoint, port, database, tls}
}
func redisIdentity(providerID, endpoint string, port int, database string) dataIdentity {
	return dataIdentity{providerID, endpoint, port, database, ""}
}

func resourcesOfType(all []pulumi.MockResourceArgs, token string) []pulumi.MockResourceArgs {
...
func onlyResource(t *testing.T, all []pulumi.MockResourceArgs, token string) pulumi.MockResourceArgs {
	t.Helper()
	matches := resourcesOfType(all, token)
	if len(matches) != 1 {
		t.Fatalf("%s registrations = %d, want 1", token, len(matches))
	}
	return matches[0]
}
func managedUpstash(t *testing.T, all []pulumi.MockResourceArgs) pulumi.MockResourceArgs {
	t.Helper()
	matches := resourcesOfType(all, "upstash:index/redisDatabase:RedisDatabase")
	if len(matches) != 1 {
		t.Fatalf("managed Upstash registrations = %d, want 1", len(matches))
	}
	return matches[0]
}
func hostForServer(t *testing.T, hosts []pulumi.MockResourceArgs, server string) pulumi.MockResourceArgs {
	t.Helper()
	for _, host := range hosts {
		if stringValue(t, property(object(t, host.Inputs, "resource"), "serverKey")) == server {
			return host
		}
	}
	t.Fatalf("Host for serverKey %q not found", server)
	return pulumi.MockResourceArgs{}
}
func object(t *testing.T, values resource.PropertyMap, key string) resource.PropertyMap {
	t.Helper()
	value, ok := values[resource.PropertyKey(key)]
	value = unwrap(value)
	if !ok || !value.IsObject() {
		t.Fatalf("%q is not an object: %v", key, value)
	}
	return value.ObjectValue()
}
func assertCount(t *testing.T, all []pulumi.MockResourceArgs, token string, want int) {
	t.Helper()
	if got := len(resourcesOfType(all, token)); got != want {
		t.Fatalf("%s registrations = %d, want %d", token, got, want)
	}
}
func assertNoCalls(t *testing.T, mocks *recordingMocks) {
	t.Helper()
	if len(mocks.calls) != 0 {
		t.Fatalf("Pulumi Call count = %d, want 0", len(mocks.calls))
	}
}
...
func directDependencies(item pulumi.MockResourceArgs) []string {
	if item.RegisterRPC == nil {
		return nil
	}
	return item.RegisterRPC.GetDependencies()
}
...
func assertHostDependsOn(t *testing.T, dependent, prerequisite pulumi.MockResourceArgs) {
	t.Helper()
	if !containsURN(directDependencies(dependent), prerequisite) {
		t.Fatalf("Host %q must depend on Host %q: %v", dependent.Name, prerequisite.Name, directDependencies(dependent))
	}
}
func assertHostDoesNotDependOn(t *testing.T, dependent, other pulumi.MockResourceArgs) {
	t.Helper()
	if containsURN(directDependencies(dependent), other) {
		t.Fatalf("Host %q must not depend on Host %q", dependent.Name, other.Name)
	}
}
func assertPropertyDependency(t *testing.T, dependent, prerequisite pulumi.MockResourceArgs) {
	t.Helper()
	if dependent.RegisterRPC == nil {
		t.Fatalf("%q lacks RegisterRPC", dependent.Name)
	}
	for _, dependencies := range dependent.RegisterRPC.GetPropertyDependencies() {
		if containsURN(dependencies.GetUrns(), prerequisite) {
			return
		}
	}
	t.Fatalf("%q inputs lack a property dependency on %q", dependent.Name, prerequisite.Name)
}
func assertHostBase(t *testing.T, host pulumi.MockResourceArgs, server, alias string) {
	t.Helper()
	if stringValue(t, property(object(t, host.Inputs, "resource"), "serverKey")) != server || stringValue(t, property(object(t, host.Inputs, "server"), "sshAlias")) != alias {
		t.Fatalf("Host input identity = %v", host.Inputs)
	}
	for _, key := range []string{"resource", "server", "target", "secrets"} {
		if _, ok := host.Inputs[resource.PropertyKey(key)]; !ok {
			t.Fatalf("Host %q missing %q", server, key)
		}
	}
	if !host.Inputs["secrets"].IsSecret() {
		t.Fatalf("Host %q secrets are not secret", server)
	}
}
func property(values resource.PropertyMap, key string) resource.PropertyValue {
	return values[resource.PropertyKey(key)]
}
...
func stringValue(t *testing.T, value resource.PropertyValue) string {
	t.Helper()
	value = unwrap(value)
	if value.IsComputed() || !value.IsString() {
		t.Fatalf("value is not a known string: %v", value)
	}
	return value.StringValue()
}
func boolValue(t *testing.T, value resource.PropertyValue) bool {
	t.Helper()
	value = unwrap(value)
	if value.IsComputed() || !value.IsBool() {
		t.Fatalf("value is not a known bool: %v", value)
	}
	return value.BoolValue()
}
func optionalBool(t *testing.T, value resource.PropertyValue) bool {
	t.Helper()
	if value.V == nil {
		return false
	}
...
func appTarget(t *testing.T, inputs resource.PropertyMap, id string) resource.PropertyMap {
	t.Helper()
	apps := unwrap(property(object(t, inputs, "target"), "apps"))
	if !apps.IsArray() {
		t.Fatalf("target.apps is not an array: %v", apps)
	}
	for _, app := range apps.ArrayValue() {
		if stringValue(t, property(app.ObjectValue(), "id")) == id {
...
func dataLink(t *testing.T, app resource.PropertyMap, name string) resource.PropertyMap {
	...
	if !links.IsArray() {
		t.Fatalf("dataLinks is not an array: %v", links)
	}
	for _, link := range links.ArrayValue() {
		if stringValue(t, property(link.ObjectValue(), "name")) == name {
			return link.ObjectValue()
		}
	}
	t.Fatalf("data link %q absent", name)
	return nil
}
func assertIdentity(t *testing.T, link resource.PropertyMap, kind string, want dataIdentity) {
	t.Helper()
	identity := object(t, link, "identity")
	got := dataIdentity{stringValue(t, property(identity, "providerId")), stringValue(t, property(identity, "endpoint")), int(unwrap(property(identity, "port")).NumberValue()), stringValue(t, property(identity, "database")), optionalString(t, property(identity, "tlsServerName"))}
	if stringValue(t, property(identity, "kind")) != kind || got != want {
		t.Fatalf("%s identity = %#v / %s, want %#v / %s", stringValue(t, property(link, "name")), got, stringValue(t, property(identity, "kind")), want, kind)
	}
}
func optionalString(t *testing.T, value resource.PropertyValue) string {
	t.Helper()
	if value.V == nil {
		return ""
	}
...
func appCredentials(t *testing.T, inputs resource.PropertyMap, appID, kind string) resource.PropertyMap {
	t.Helper()
	secrets := unwrap(property(inputs, "secrets"))
	if !secrets.IsObject() {
		t.Fatalf("Host secrets are not an object: %v", secrets)
	}
	apps := object(t, secrets.ObjectValue(), "apps")
	return object(t, apps, appID)[resource.PropertyKey(kind)].ObjectValue()
}
func assertManagedApp(t *testing.T, host pulumi.MockResourceArgs, appID, postgresName string, postgres dataIdentity, redisName string, redis dataIdentity, postgresPassword, redisPassword string) {
	t.Helper()
	app := appTarget(t, host.Inputs, appID)
	assertIdentity(t, dataLink(t, app, postgresName), "postgres", postgres)
	assertIdentity(t, dataLink(t, app, redisName), "redis", redis)
	pg, rd := appCredentials(t, host.Inputs, appID, "postgres"), appCredentials(t, host.Inputs, appID, "redis")
	if stringValue(t, property(pg, "username")) != "appuser" || stringValue(t, property(pg, "password")) != postgresPassword || stringValue(t, property(rd, "username")) != "default" || stringValue(t, property(rd, "password")) != redisPassword {
		t.Fatalf("App credentials are not exact: postgres=%v redis=%v", pg, rd)
	}
}
func assertAppSchema(t *testing.T, host pulumi.MockResourceArgs, bootstrap bool) {
	t.Helper()
	app := appTarget(t, host.Inputs, "app")
	if stringValue(t, property(app, "readinessPath")) != "/ready" || stringValue(t, property(app, "drainTimeout")) != "10s" {
		t.Fatalf("App runtime defaults = %v", app)
	}
	settings := object(t, app, "runtimeSettings")
	if stringValue(t, property(settings, "FEATURE_FLAG")) != "enabled" || property(settings, "ADMIN_EMAIL").V != nil {
		t.Fatalf("App runtime settings = %v", settings)
	}
	if stringValue(t, property(app, "initialAdminEmail")) != "admin@example.test" {
		t.Fatalf("App initial admin email = %v", app)
	}
	if bootstrap != optionalBool(t, property(app, "initialBootstrap")) {
		t.Fatalf("initialBootstrap = %v, want %t", property(app, "initialBootstrap"), bootstrap)
	}
	secrets := unwrap(property(host.Inputs, "secrets")).ObjectValue()
	appSecrets := object(t, object(t, secrets, "apps"), "app")
	runtime := appSecrets["runtimeEnvironment"].ObjectValue()
	if stringValue(t, property(runtime, "PRIVATE_FLAG")) != privateFlag {
		t.Fatalf("App private environment = %v", runtime)
	}
	if bootstrap != (property(appSecrets, "initialAdminPassword").V != nil) {
		t.Fatalf("initial admin secret scope = %v, want bootstrap=%t", appSecrets, bootstrap)
	}
}
func assertDNSInputs(t *testing.T, record pulumi.MockResourceArgs) {
	t.Helper()
	inputs := record.Inputs
	if stringValue(t, property(inputs, "name")) != "app.example.test" || stringValue(t, property(inputs, "type")) != "A" || stringValue(t, property(inputs, "zoneId")) != "zone-canary" || !unwrap(property(inputs, "proxied")).BoolValue() || int(unwrap(property(inputs, "ttl")).NumberValue()) != 1 {
		t.Fatalf("DNS inputs = %v", inputs)
	}
}
func assertDNSContent(t *testing.T, record pulumi.MockResourceArgs, want string) {
	t.Helper()
	if stringValue(t, property(record.Inputs, "content")) != want {
		t.Fatalf("DNS content = %q, want %q", stringValue(t, property(record.Inputs, "content")), want)
	}
}
func providerFor(t *testing.T, dependent pulumi.MockResourceArgs, providers []pulumi.MockResourceArgs) pulumi.MockResourceArgs {
	t.Helper()
	for _, provider := range providers {
		if dependent.RegisterRPC.GetProvider() == providerReference(provider) {
			return provider
		}
	}
	t.Fatalf("no recorded provider matches %q", dependent.RegisterRPC.GetProvider())
	return pulumi.MockResourceArgs{}
}
func assertKnownHostContract(t *testing.T, host pulumi.MockResourceArgs) {
	t.Helper()
	target, secrets := decodeKnownHost(t, host)
	if err := hostcontract.ValidateTarget(target, secrets); err != nil {
		t.Fatalf("Host %q violates host contract: %v", host.Name, err)
	}
}
func decodeKnownHost(t *testing.T, host pulumi.MockResourceArgs) (hostcontract.Target, hostcontract.Secrets) {
	t.Helper()
	targetJSON := propertyJSON(t, property(host.Inputs, "target"))
	secretsJSON := propertyJSON(t, property(host.Inputs, "secrets"))
	var target hostcontract.Target
	var secrets hostcontract.Secrets
	if err := json.Unmarshal(targetJSON, &target); err != nil {
		t.Fatalf("decode Host target: %v", err)
	}
	if err := json.Unmarshal(secretsJSON, &secrets); err != nil {
		t.Fatalf("decode Host secrets: %v", err)
	}
	return target, secrets
}
func propertyJSON(t *testing.T, value resource.PropertyValue) []byte {
	t.Helper()
	value = unwrap(value)
	if containsComputed(value) {
		t.Fatalf("cannot decode computed Host input: %v", value)
	}
	normalized := normalizeProperty(value)
	bytes, err := json.Marshal(normalized)
	if err != nil {
		t.Fatalf("encode Host input: %v", err)
	}
	return bytes
}
func containsComputed(value resource.PropertyValue) bool {
	value = unwrap(value)
	if value.IsComputed() {
		return true
	}
...
func normalizeProperty(value resource.PropertyValue) any {
	value = unwrap(value)
	if value.IsArray() {
		result := make([]any, len(value.ArrayValue()))
		for i, child := range value.ArrayValue() {
			result[i] = normalizeProperty(child)
		}
		return result
	}
	if value.IsObject() {
		result := map[string]any{}
		for key, child := range value.ObjectValue() {
			result[string(key)] = normalizeProperty(child)
		}
		return result
	}
	return value.V
}
func assertLocalServices(t *testing.T, host pulumi.MockResourceArgs) {
	t.Helper()
	services := unwrap(property(object(t, host.Inputs, "target"), "dataServices"))
	if !services.IsArray() || len(services.ArrayValue()) != 2 {
		t.Fatalf("local data services = %v, want two", services)
	}
	byID := map[string]resource.PropertyMap{}
	for _, service := range services.ArrayValue() {
		byID[stringValue(t, property(service.ObjectValue(), "id"))] = service.ObjectValue()
	}
	pg, rd := byID["app-postgres"], byID["app-redis"]
	if stringValue(t, property(pg, "type")) != "postgres" || int(unwrap(property(pg, "port")).NumberValue()) != 5432 || stringValue(t, property(rd, "type")) != "redis" || int(unwrap(property(rd, "port")).NumberValue()) != 6379 || !unwrap(property(rd, "persistence")).BoolValue() {
		t.Fatalf("local data services = %v", byID)
	}
	secrets := unwrap(property(host.Inputs, "secrets")).ObjectValue()
	local := object(t, secrets, "localDataServices")
	if len(local) != 2 || stringValue(t, property(object(t, local, "app-postgres"), "adminPassword")) != postgresAdmin || stringValue(t, property(object(t, local, "app-redis"), "adminPassword")) != redisAdmin {
		t.Fatalf("local data service secrets = %v", local)
	}
}
func assertNoApps(t *testing.T, host pulumi.MockResourceArgs) {
	t.Helper()
	target := object(t, host.Inputs, "target")
	if apps, ok := target["apps"]; ok {
		apps = unwrap(apps)
		if apps.IsArray() && len(apps.ArrayValue()) != 0 {
			t.Fatalf("Host %q unexpectedly projects Apps: %v", host.Name, apps)
		}
	}
}
func assertNoRuntimeSecrets(t *testing.T, host pulumi.MockResourceArgs) {
	t.Helper()
	secrets := unwrap(property(host.Inputs, "secrets"))
	if !secrets.IsObject() {
		t.Fatalf("Host %q secrets are not an object", host.Name)
	}
	for _, key := range []string{"apps", "localDataServices"} {
		if value, ok := secrets.ObjectValue()[resource.PropertyKey(key)]; ok && unwrap(value).IsObject() && len(unwrap(value).ObjectValue()) != 0 {
			t.Fatalf("Host %q must not receive %s secrets: %v", host.Name, key, value)
		}
	}
}
func assertHostSecrets(t *testing.T, host pulumi.MockResourceArgs, present, absent []string) {
	t.Helper()
	secrets := property(host.Inputs, "secrets")
	for _, canary := range present {
		if !containsProperty(secrets, canary) {
			t.Fatalf("Host %q lacks required secret %q", host.Name, canary)
		}
	}
	for _, canary := range absent {
		if containsProperty(secrets, canary) {
			t.Fatalf("Host %q leaked forbidden secret %q", host.Name, canary)
		}
	}
}
func assertProviderToken(t *testing.T, inputs resource.PropertyMap, key, canary string) {
	t.Helper()
	value := property(inputs, key)
	if !containsSecretProperty(value, canary) || occurrences(value, canary) != 1 {
		t.Fatalf("provider %s must contain exactly one secret %q: %v", key, canary, value)
	}
}
func assertNotContains(t *testing.T, inputs resource.PropertyMap, canaries ...string) {
	t.Helper()
	for _, canary := range canaries {
		if containsProperty(resource.NewObjectProperty(inputs), canary) {
			t.Fatalf("secret %q leaked into ordinary resource input", canary)
		}
	}
}
...
func containsSecretProperty(value resource.PropertyValue, want string) bool {
	if value.IsSecret() {
		return containsProperty(value.SecretValue().Element, want) || containsSecretProperty(value.SecretValue().Element, want)
	}
	if value.IsArray() {
		for _, child := range value.ArrayValue() {
			if containsSecretProperty(child, want) {
				return true
			}
		}
	}
...
func occurrences(value resource.PropertyValue, want string) int {
	if value.IsString() {
	...
	if value.IsSecret() {
		return occurrences(value.SecretValue().Element, want)
	}
	if value.IsComputed() {
		return 0
	}
...
func canonicalSnapshot(resources []pulumi.MockResourceArgs) string {
...
func semanticIdentity(item pulumi.MockResourceArgs) string {
	if item.TypeToken == hostresource.HostToken {
		if value, ok := item.Inputs["resource"]; ok && unwrap(value).IsObject() {
			return stringValueNoFail(unwrap(value).ObjectValue()["serverKey"])
		}
	}
	if id, ok := item.Inputs["name"]; ok && unwrap(id).IsString() {
		return stringValueNoFail(unwrap(id))
	}
	if id, ok := item.Inputs["databaseName"]; ok && unwrap(id).IsString() {
		return stringValueNoFail(unwrap(id))
	}
	return item.Name
}
func stringValueNoFail(value resource.PropertyValue) string {
	value = unwrap(value)
	if value.IsComputed() || !value.IsString() {
		return "<unknown>"
	}
...
func providerToken(item pulumi.MockResourceArgs, all []pulumi.MockResourceArgs) string {
	if item.RegisterRPC == nil {
		return ""
	}
	for _, provider := range all {
		if item.RegisterRPC.GetProvider() == providerReference(provider) {
			return provider.TypeToken
		}
	}
	return ""
}
func normalizedDependencies(item pulumi.MockResourceArgs, all []pulumi.MockResourceArgs) []string {
	var result []string
	for _, urn := range directDependencies(item) {
		for _, dependency := range all {
			if strings.Contains(urn, "::"+dependency.Name+"::") {
				result = append(result, dependency.TypeToken+":"+semanticIdentity(dependency))
			}
		}
	}
	sort.Strings(result)
...
func sanitize(value resource.PropertyMap) string {
	var scrub func(resource.PropertyValue) any
	scrub = func(value resource.PropertyValue) any {
		if value.IsSecret() {
			return scrub(value.SecretValue().Element)
		}
		if value.IsComputed() {
			return "<computed>"
		}
...
func reorderedExternalConfig() string {
	return strings.Replace(externalConfig(), "  alpha:\n    sshAlias: alpha-ssh\n    addresses:\n      public:\n        ipv4: 198.51.100.11\n  bravo:\n    sshAlias: bravo-ssh\n    addresses:\n      public:\n        ipv4: 198.51.100.12", "  bravo:\n    sshAlias: bravo-ssh\n    addresses:\n      public:\n        ipv4: 198.51.100.12\n  alpha:\n    sshAlias: alpha-ssh\n    addresses:\n      public:\n        ipv4: 198.51.100.11", 1)
}
func maintenanceConfig() string {
	return strings.Replace(externalConfig(), "servers: [alpha, bravo]", "servers: []", 1)
}
func managedMaintenanceConfig() string {
	config := strings.Replace(managedConfig(), "cloudflare:\n  zoneId: zone-canary\n", "", 1)
	config = strings.Replace(config, "type: cloudflare\n      servers: [alpha, bravo]\n      cloudflare:\n        mode: dns\n        connectBy: publicAddress", "type: none", 1)
	return strings.Replace(config, "servers: [alpha, bravo]", "servers: []", 1)
}
func dockerMaintenanceConfig() string {
	return strings.Replace(sameHostDockerConfig(), "servers: [alpha]", "servers: []", 1)
}
func appDrainTimeoutConfig(value string) string {
	return strings.Replace(externalConfig(), "    readinessPath: /ready\n", "    readinessPath: /ready\n    drainTimeout: "+value+"\n", 1)
}
func crossKindDockerCollisionConfig() string {
	config := strings.Replace(sameHostDockerConfig(), "  app-postgres:\n", "  shared-data:\n", 1)
	config = strings.Replace(config, "  app-redis:\n", "  shared-data:\n", 1)
	config = strings.Replace(config, "name: app-postgres", "name: shared-data", 1)
	return strings.Replace(config, "name: app-redis", "name: shared-data", 1)
}
func crossKindDockerCollisionSecrets() string {
	secrets := strings.Replace(dockerSecrets(), "  app-postgres:\n", "  shared-data:\n", 1)
	return strings.Replace(secrets, "  app-redis:\n", "  shared-data:\n", 1)
}
func sameHostDockerConfig() string {
	return strings.Replace(baseConfig("docker", "docker", "none"), "servers: [alpha, bravo]", "servers: [alpha]", 1)
}
func crossHostDockerConfig(kind string) string {
	config := baseConfig("external", "external", "none")
	if kind == "postgres" {
	...
	config = strings.Replace(config, "      public:\n        ipv4: 198.51.100.11", "      public:\n        ipv4: 198.51.100.11\n      internal:\n        ipv4: 10.0.0.11", 1)
	return strings.Replace(config, "      public:\n        ipv4: 198.51.100.12", "      public:\n        ipv4: 198.51.100.12\n      internal:\n        ipv4: 10.0.0.12", 1)
}
func duplicateHostnameConfig() string {
	return externalConfig() + "  app-two:\n    hostname: app.example.test\n    image: ghcr.io/example/sub2api@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd\n    initialAdminEmail: admin-two@example.test\n    readinessPath: /ready\n    servers: [alpha]\n    postgres: {name: app-postgres, database: sub2api-two}\n    redis: {name: app-redis, database: 1}\n    publicAccess: {type: none}\n"
}
func duplicateHostnameSecrets() string {
	return externalSecrets() + "  app-two:\n    initialAdminPassword: app-two-admin-password-canary\n    jwtSecret: app-two-jwt-secret-canary\n    totpEncryptionKey: app-two-totp-key-canary\n    postgres: {username: appuser, password: app-two-postgres-password-canary}\n    redis: {username: default, password: app-two-redis-password-canary}\n"
}
func managedNeonConfig() string {
	return strings.Replace(managedConfig(), "type: external\n    host: postgres.external.test\n    port: 5433\n    tls: {mode: verify-full}", "type: neon\n    region: aws-us-east-1", 1)
}
func managedNeonSecrets() string {
	return strings.Replace(managedSecrets(), "    postgres: {username: appuser, password: external-postgres-password-canary}\n", "", 1) + "postgres:\n  app-postgres:\n    apiToken: neon-api-token-canary\n"
}
func enabledOutboundProxyConfig() string {
	return externalConfig() + "    outboundProxy:\n      enabled: true\n      type: microsocks\n      required: true\n      servers: [alpha]\n"
}
func singBoxConfig() string {
	return strings.Replace(externalConfig(), "    addresses:\n      public:\n        ipv4: 198.51.100.11", "    addresses:\n      public:\n        ipv4: 198.51.100.11\n    singBox:\n      serverName: proxy.example.test\n      target: proxy.example.test:443", 1)
}
func twoUpstashConfig() string {
	config := strings.Replace(managedConfig(), "\napps:\n", "\n  app-redis-two:\n    type: upstash\n    region: us-east-1\napps:\n", 1)
	return config + "  app-two:\n    hostname: app-two.example.test\n    image: ghcr.io/example/sub2api@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd\n    initialAdminEmail: admin-two@example.test\n    readinessPath: /ready\n    servers: [bravo]\n    postgres: {name: app-postgres, database: sub2api-two}\n    redis: {name: app-redis-two, database: 1}\n    publicAccess: {type: none}\n"
}
func baseConfig(postgres, redis, access string) string {
	...
	public, cloudflare := "type: none", ""
	if access == "cloudflare" {
		public, cloudflare = "type: cloudflare\n      servers: [alpha, bravo]\n      cloudflare:\n        mode: dns\n        connectBy: publicAddress", "cloudflare:\n  zoneId: zone-canary\n"
	}
...
func managedNoPublicSecrets() string {
	return strings.Replace(managedSecrets(), "cloudflare:\n  apiToken: "+cloudflareToken+"\n", "", 1)
}
func enabledOutboundProxySecrets() string {
	return externalSecrets() + "    adminApiKey: app-admin-api-key-canary\noutboundProxy:\n  alpha: {username: proxy-user-canary, password: proxy-password-canary}\n"
}
func twoUpstashSecrets() string {
	secrets := strings.Replace(managedSecrets(), "\nredis:\n", "\n  app-two:\n    initialAdminPassword: app-two-admin-password-canary\n    jwtSecret: app-two-jwt-secret-canary\n    totpEncryptionKey: app-two-totp-key-canary\n    postgres: {username: appuser, password: "+postgresPassword+"}\nredis:\n", 1)
	return strings.Replace(secrets, "  app-redis:\n    apiKey: "+upstashToken+"\n", "  app-redis:\n    apiKey: "+upstashToken+"\n  app-redis-two:\n    apiKey: "+upstashTwoToken+"\n", 1)
}
func crossHostDockerSecrets(local string) string {
	pg, rd, services := "    postgres: {username: appuser, password: "+postgresPassword+"}\n", "    redis: {username: default, password: "+redisPassword+"}\n", ""
	if local == "postgres" {
		pg = "    postgres: {username: appuser, password: app-postgres-password-canary}\n"
		services = "postgres:\n  app-postgres:\n    adminPassword: " + postgresAdmin + "\n"
	} else {
		rd = "    redis: {username: default, password: app-redis-password-canary}\n"
		services = "redis:\n  app-redis:\n    adminPassword: " + redisAdmin + "\n"
	}
	return appSecrets(pg, rd) + services
}
func secrets(upstash, docker bool) string {
	pg, rd, services := "    postgres: {username: appuser, password: "+postgresPassword+"}\n", "    redis: {username: default, password: "+redisPassword+"}\n", ""
	cloudflare := ""
	if upstash {
		cloudflare, rd, services = "cloudflare:\n  apiToken: "+cloudflareToken+"\n", "", "redis:\n  app-redis:\n    apiKey: "+upstashToken+"\n"
	}
	if docker {
		pg, rd, services = "    postgres: {username: appuser, password: app-postgres-password-canary}\n", "    redis: {username: default, password: app-redis-password-canary}\n", "postgres:\n  app-postgres:\n    adminPassword: "+postgresAdmin+"\nredis:\n  app-redis:\n    adminPassword: "+redisAdmin+"\n"
	}
	return cloudflare + appSecrets(pg, rd) + services
}
func appSecrets(pg, rd string) string {
	return "revisionKey: MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=\nreverseProxy:\n  dnsChallengeToken: " + dnsToken + "\napps:\n  app:\n    initialAdminPassword: " + adminPassword + "\n    jwtSecret: " + jwtSecret + "\n    totpEncryptionKey: " + totpKey + "\n    environment: {PRIVATE_FLAG: " + privateFlag + "}\n" + pg + rd
}
