...
func TestTokenRoundTripDeterministicAndPayloadBound(t *testing.T) {
	...
	if err != nil || !deepEqual(got, inputs) {
		t.Fatalf("round trip failed: %#v %v", got, err)
	}
	retry, err := Encode(tokenKey, inputs)
	if err != nil || retry != token {
		t.Fatal("identical payload did not retain a stable token")
	}
	different := inputs
	different.Server.SSHAlias = "other"
	other, err := Encode(tokenKey, different)
	if err != nil || other == token {
		t.Fatal("different payload did not receive a distinct token")
	}
}
...
func TestTokenRejectsAdversarialEnvelopeBeforeExposingPayload(t *testing.T) {
	...
	if _, err := Decode(hostcontract.RevisionKey([]byte("abcdefghijklmnopqrstuvwxyz012345")), token); err == nil {
		t.Fatal("wrong revision key accepted")
	}
}
...
func TestTokenNormalizesOrderAndNilEquivalenceAtBounds(t *testing.T) {
	...
	first.Target.Apps = []hostcontract.AppTarget{
		{ID: "bravo", Image: "bravo", Hostname: "bravo.example", ReadinessPath: "/ready", InitialAdminEmail: "bravo@example.test"},
		{ID: "alpha", Image: "alpha", Hostname: "alpha.example", ReadinessPath: "/ready", InitialAdminEmail: "alpha@example.test"},
	}
	second := tokenInputs()
	...
	if err != nil || orderedToken != reorderedToken {
		t.Fatal("equivalent ordered payloads were not canonical")
	}
	...
	firstToken, err := Encode(tokenKey, first)
	if err != nil {
		t.Fatal(err)
	}
	secondToken, err := Encode(tokenKey, second)
	if err != nil || firstToken != secondToken {
		t.Fatal("nil-equivalent payload was not canonical")
	}
	if _, err := Encode(hostcontract.RevisionKey([]byte("short")), first); err == nil {
...
func tokenInputs() Inputs {
	return Inputs{Resource: hostcontract.ResourceIdentity{Environment: "canary", ServerKey: "edge"}, Server: hostcontract.ServerTarget{SSHAlias: "edge"}, Target: hostcontract.Target{ReleaseArtifact: "image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}, Secrets: hostcontract.Secrets{}}
}
func deepEqual(a, b Inputs) bool {
	a.Target, a.Secrets = hostcontract.NormalizeTargetSecrets(a.Target, a.Secrets)
	b.Target, b.Secrets = hostcontract.NormalizeTargetSecrets(b.Target, b.Secrets)
	return string(mustJSON(a)) == string(mustJSON(b))
}
func mustJSON(value any) []byte { valueJSON, _ := json.Marshal(value); return valueJSON }
func assertInvalidToken(t *testing.T, value string) {
	t.Helper()
	if _, err := Decode(tokenKey, value); err == nil || strings.Contains(err.Error(), "CANARY") {
		t.Fatalf("invalid token accepted or exposed data: %v", err)
	}
}
func assertInvalidEncode(t *testing.T, inputs Inputs) {
	t.Helper()
	if _, err := Encode(tokenKey, inputs); err == nil || strings.Contains(err.Error(), "CANARY") {
		t.Fatalf("invalid token inputs accepted or exposed data: %v", err)
	}
}
func encryptPlaintext(t *testing.T, plain []byte) string {
	t.Helper()
	pid := testMAC(domainID, plain)
	block, err := aes.NewCipher(testMAC(domainKey, pid))
	if err != nil {
		t.Fatal(err)
	}
	...
	raw := append(pid, gcm.Seal(nil, testMAC(domainNonce, pid)[:12], plain, aad)...)
	return tokenPrefix + base64.RawURLEncoding.EncodeToString(raw)
}
func testMAC(domain, payload []byte) []byte {
