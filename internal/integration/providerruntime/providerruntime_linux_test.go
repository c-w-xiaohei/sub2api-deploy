...
func createInputsWithRelease(release string) property.Map {
	target := hostcontract.Target{ReleaseArtifact: release, Apps: []hostcontract.AppTarget{{ID: "api", Image: "api@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", Hostname: "api.example", ReadinessPath: "/ready", InitialAdminEmail: "admin@example.test"}}}
...
func createInputsWithImage(image string) property.Map {
	inputs := createInputs()
	target := hostcontract.Target{ReleaseArtifact: ciRelease, Apps: []hostcontract.AppTarget{{ID: "api", Image: image, Hostname: "api.example", ReadinessPath: "/ready", InitialAdminEmail: "admin@example.test"}}}
...
func frozenCopyAppSecrets(values map[string]hostcontract.AppSecrets) map[string]hostcontract.AppSecrets {
	...
	for k, v := range values {
		v.RuntimeEnvironment = frozenCopyStrings(v.RuntimeEnvironment)
		copy[k] = v
	}
	return copy
}
...
func assertFrozenNormalizationOracle(t *testing.T) {
	...
	gotJSON, gotErr := json.Marshal(struct {
		Target  hostcontract.Target  `json:"target"`
		Secrets hostcontract.Secrets `json:"secrets"`
	}{gotTarget, gotSecrets})
	wantJSON, wantErr := json.Marshal(struct {
		Target  hostcontract.Target  `json:"target"`
		Secrets hostcontract.Secrets `json:"secrets"`
	}{wantTarget, wantSecrets})
...
func expectedInventoryEffect(t *testing.T, h *providerProcess, action string, app inventoryApp) dockerEffect {
	...
	ownerSum, targetSum := sha256.Sum256([]byte(owner)), sha256.Sum256([]byte(target))
	if app.Name != name {
		t.Fatalf("pre-update inventory name = %q, want %q", app.Name, name)
	}
