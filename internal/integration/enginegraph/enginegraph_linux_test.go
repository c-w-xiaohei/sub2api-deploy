...
var crossHostFixtureSecretCanaries = []string{
	"MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=",
	"cloudflare-api-token-canary",
	"dns-challenge-token-canary",
	"app-admin-password-canary",
	"app-jwt-secret-canary",
	"app-totp-key-canary",
	"private-flag-canary",
	"app-postgres-password-canary",
	"app-redis-password-canary",
	"postgres-admin-password-canary",
	"redis-admin-password-canary",
}
...
func TestEngineGraphCrossHostDataAdmissionOrderingAndFailureStop(t *testing.T) {
	const configName = "cross-host-docker-bravo-cloudflare.yaml"
	const secretsName = "cross-host-docker-bravo-cloudflare-secrets.yaml"

	t.Run("admission then App then publication", func(t *testing.T) {
		harness := newEngineGraphHarness(t, map[string]bool{"alpha": true, "bravo": true})
		snapshot, err := harness.update(t, configName, secretsName)
		if err != nil {
			t.Fatalf("cross-Host data update: %v", err)
		}
		assertCrossHostTargets(t, snapshot, true, true, true)
		events := harness.trace.snapshot()
		if !slices.Equal(events, []string{"host:alpha:create:ok", "host:bravo:create:ok", "cloudflare:dns:dns-app-bravo-A:create:ok"}) {
			t.Fatalf("cross-Host lifecycle trace = %v, want data admission, App readiness, then publication", events)
		}
		assertSanitizedLifecycleTrace(t, events)
	})

	t.Run("data failure stops App and publication", func(t *testing.T) {
		trace, _, err := runEngineGraphUpdate(t, configName, secretsName, map[string]bool{"alpha": false, "bravo": true})
		if err == nil || !strings.Contains(err.Error(), scriptedAlphaFailure) {
			t.Fatalf("data Host failure = %v, want %q", err, scriptedAlphaFailure)
		}
		if got := trace.snapshot(); !slices.Equal(got, []string{alphaFailureTrace}) {
			t.Fatalf("data failure trace = %v, want no bravo write or publication", got)
		}
		if got := trace.publicationSnapshot(); len(got) != 0 {
			t.Fatalf("data failure publication trace = %v, want none", got)
		}
	})

	t.Run("App failure stops publication", func(t *testing.T) {
		harness := newEngineGraphHarness(t, map[string]bool{"alpha": true, "bravo": false})
		_, err := harness.update(t, configName, secretsName)
		if err == nil || !strings.Contains(err.Error(), scriptedBravoFailure) {
			t.Fatalf("App Host failure = %v, want %q", err, scriptedBravoFailure)
		}
		if got := harness.trace.snapshot(); !slices.Equal(got, []string{"host:alpha:create:ok", "host:bravo:create:fail"}) {
			t.Fatalf("App failure trace = %v, want successful data admission and no publication", got)
		}
		if got := harness.trace.publicationSnapshot(); len(got) != 0 {
			t.Fatalf("App failure publication trace = %v, want none", got)
		}
	})
}

func TestEngineGraphCrossHostDataRemovalIsReverseStaged(t *testing.T) {
	const initialConfig = "cross-host-docker-bravo-cloudflare.yaml"
	const initialSecrets = "cross-host-docker-bravo-cloudflare-secrets.yaml"
	const finalConfig = "cross-host-docker-bravo-removed.yaml"
	const finalSecrets = "cross-host-docker-bravo-removed-secrets.yaml"
	const bravoURN = "urn:pulumi:canary::sub2api-environment::sub2api-host:index:Host::host-bravo"
	const publicationURN = "urn:pulumi:canary::sub2api-environment::cloudflare:index/dnsRecord:DnsRecord::dns-app-bravo-A"

	harness := newEngineGraphHarness(t, map[string]bool{"alpha": true, "bravo": true})
	initial, err := harness.update(t, initialConfig, initialSecrets)
	if err != nil {
		t.Fatalf("initial cross-Host data update: %v", err)
	}
	assertCrossHostTargets(t, initial, true, true, true)

	before := len(harness.trace.snapshot())
	publication, err := harness.updateTargets(t, finalConfig, finalSecrets, publicationURN)
	if err != nil {
		t.Fatalf("publication detach: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("bravo App detachment: %v", err)
	}
	assertCrossHostTargets(t, detached, false, true, false)
	assertHostAppCount(t, detached, "host-bravo", 0)
	assertStageTrace(t, harness.trace.snapshot()[before:], []string{"host:bravo:update:ok"})
	before = len(harness.trace.snapshot())
	detachedRetry, err := harness.updateTargets(t, finalConfig, finalSecrets, bravoURN)
	if err != nil {
		t.Fatalf("bravo App detachment retry: %v", err)
	}
	assertSemanticCheckpointEqual(t, detachedRetry, detached)
	assertCrossHostTargets(t, detachedRetry, false, true, false)
	assertStageTrace(t, harness.trace.snapshot()[before:], nil)

	before = len(harness.trace.snapshot())
	final, err := harness.update(t, finalConfig, finalSecrets)
	if err != nil {
		t.Fatalf("final data-policy contraction: %v", err)
	}
	assertCrossHostTargets(t, final, false, false, false)
	assertStageTrace(t, harness.trace.snapshot()[before:], []string{"host:alpha:update:ok"})

	before = len(harness.trace.snapshot())
	finalRetry, err := harness.update(t, finalConfig, finalSecrets)
	if err != nil {
		t.Fatalf("final-stage retry: %v", err)
	}
	assertSemanticCheckpointEqual(t, finalRetry, final)
	assertCrossHostTargets(t, finalRetry, false, false, false)
	assertStageTrace(t, harness.trace.snapshot()[before:], nil)
	assertSanitizedLifecycleTrace(t, harness.trace.snapshot())
}

func TestEngineGraphTraceArtifactIsSanitizedJSONL(t *testing.T) {
	directory := t.TempDir()
	t.Setenv("ENGINE_GRAPH_TRACE_DIR", directory)
	t.Cleanup(func() {
		assertPersistedEngineGraphTrace(t, directory, t.Name(), []engineGraphTraceEnvelope{
			{TestName: t.Name(), LifecycleEvent: "host:alpha:create:ok"},
			{TestName: t.Name(), LifecycleEvent: "host:bravo:create:ok"},
			{TestName: t.Name(), LifecycleEvent: "cloudflare:dns:dns-app-bravo-A:create:ok"},
			{TestName: t.Name(), PublicationEvent: "cloudflare:dns:create"},
		})
	})

	harness := newEngineGraphHarness(t, map[string]bool{"alpha": true, "bravo": true})
	snapshot, err := harness.update(t, "cross-host-docker-bravo-cloudflare.yaml", "cross-host-docker-bravo-cloudflare-secrets.yaml")
	if err != nil {
		t.Fatalf("artifact trace update: %v", err)
	}
	assertCrossHostTargets(t, snapshot, true, true, true)
	assertSanitizedLifecycleTrace(t, harness.trace.snapshot())
}
...
func assertCrossHostTargets(t *testing.T, snapshot *deploy.Snapshot, bravoHasApp, alphaAdmitsBravo, publication bool) {
	...
	if bravoHasApp {
		...
		for _, link := range []string{"app-postgres", "app-redis"} {
			if !ok || !containsString(dataLinks, link) || !containsString(dataLinks, "10.42.0.11") {
				t.Fatalf("bravo App target lacks remote %s data binding", link)
			}
		}
	}
	if wantDNS := 0; publication {
		wantDNS = 1
		if dnsRecords != wantDNS {
			t.Fatalf("cross-Host publication records = %d, want %d", dnsRecords, wantDNS)
		}
	} else if dnsRecords != 0 {
		t.Fatalf("cross-Host publication records = %d, want none", dnsRecords)
	}
	...
	if !alphaAdmitsBravo && containsString(dataServices, "10.42.0.12") {
		t.Fatalf("final alpha data policy still admits bravo: %v", dataServices)
	}
}
...
type checkpointResourceState struct {
	Type                    tokens.Type
	ID                      resource.ID
	Custom                  bool
...
func checkpointResourceStates(snapshot *deploy.Snapshot) map[resource.URN]checkpointResourceState {
	...
	for _, state := range snapshot.Resources {
		states[state.URN] = checkpointResourceState{
			...
			ID:                      state.ID,
			Custom:                  state.Custom,
			Inputs:                  clonePropertyMap(state.Inputs),
			Outputs:                 clonePropertyMap(state.Outputs),
			Parent:                  state.Parent,
			Dependencies:            append([]resource.URN(nil), state.Dependencies...),
			InitErrors:              append([]string(nil), state.InitErrors...),
...
func assertHostAppCount(t *testing.T, snapshot *deploy.Snapshot, hostID string, want int) {
	...
	t.Fatalf("checkpoint does not contain %s", hostID)
}

func assertStageTrace(t *testing.T, got, want []string) {
	t.Helper()
	if !slices.Equal(got, want) {
		t.Fatalf("stage lifecycle trace = %v, want exactly %v", got, want)
	}
}

func assertSanitizedLifecycleTrace(t *testing.T, events []string) {
	t.Helper()
	for _, secret := range crossHostFixtureSecretCanaries {
		for _, event := range events {
			if strings.Contains(event, secret) {
				t.Fatalf("secret leaked into lifecycle trace: %q", event)
			}
		}
	}
}

func assertPersistedEngineGraphTrace(t *testing.T, directory, testName string, want []engineGraphTraceEnvelope) {
	t.Helper()
	entries, err := os.ReadDir(directory)
	if err != nil {
		t.Fatalf("read trace artifact directory: %v", err)
	}
	wantFile := sanitizedTraceFileName(testName)
	if len(entries) != 1 || entries[0].Name() != wantFile || entries[0].IsDir() {
		t.Fatalf("trace artifact inventory = %v, want exactly %q", entries, wantFile)
	}
	contents, err := os.ReadFile(filepath.Join(directory, wantFile))
	if err != nil {
		t.Fatalf("read trace artifact: %v", err)
	}
	for _, forbidden := range append(append([]string(nil), crossHostFixtureSecretCanaries...), "\"target\"", "\"inputs\"") {
		if strings.Contains(string(contents), forbidden) {
			t.Fatalf("persisted trace leaks forbidden value %q", forbidden)
		}
	}
	decoder := json.NewDecoder(strings.NewReader(string(contents)))
	decoder.DisallowUnknownFields()
	var got []engineGraphTraceEnvelope
	for {
		var record engineGraphTraceEnvelope
		err := decoder.Decode(&record)
		if errors.Is(err, io.EOF) {
			break
		}
		if err != nil {
			t.Fatalf("strict JSONL trace decode: %v", err)
		}
		if record.TestName == "" || (record.LifecycleEvent == "" && record.PublicationEvent == "" && record.Summary == "") || (record.LifecycleEvent != "" && record.PublicationEvent != "") || record.Summary != "" {
			t.Fatalf("invalid sanitized trace record: %#v", record)
		}
		got = append(got, record)
	}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("persisted JSONL trace = %#v, want %#v", got, want)
	}
}
